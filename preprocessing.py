...
def process_subject(subject_path, size, overlap, logger):
    ...
    fn = f'{path.splitext(path.basename(subject_path))[0]}.h5'
...
if __name__=='__main__':
    ...
    if (len(subjects)>0):
        ...
        for i, s in enumerate(tqdm(subjects)):
            if (path.isfile(s)):
                saved_path, unlabel_shape, label_shape = process_subject(s, args.size, args.overlap, logger)
